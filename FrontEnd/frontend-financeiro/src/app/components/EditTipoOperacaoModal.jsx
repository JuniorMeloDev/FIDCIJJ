        ...
        if (isOpen) {
            if (tipoOperacao) {
                setFormData({
                    ...
                    descricao: tipoOperacao.descricao || '', // Adicionado
                });
            } else {
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input type="text" name="nome" value={formData.nome} onChange={handleChange} className="mt-1 block w-full border-gray-300 rounded-md p-2 text-sm"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Taxa de Juros (%)</label>
                        <input type="text" name="taxaJuros" placeholder="Ex: 2,5" value={formData.taxaJuros} onChange={handleChange} className="mt-1 block w-full border-gray-300 rounded-md p-2 text-sm"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Valor Fixo (R$)</label>
                        <input type="text" name="valorFixo" placeholder="R$ 0,00" value={formData.valorFixo} onChange={handleChange} className="mt-1 block w-full border-gray-300 rounded-md p-2 text-sm"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Despesas Bancárias (R$)</label>
                        <input type="text" name="despesasBancarias" placeholder="R$ 0,00" value={formData.despesasBancarias} onChange={handleChange} className="mt-1 block w-full border-gray-300 rounded-md p-2 text-sm"/>
                    </div>
                ...
                <div className="mt-6 flex justify-between border-t pt-4">
                    <div>
                        {isEditMode && (
                             <button onClick={() => onDelete(tipoOperacao.id)} className="bg-red-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-red-700 text-sm">
                                Excluir
                            </button>
                        )}
                    </div>
