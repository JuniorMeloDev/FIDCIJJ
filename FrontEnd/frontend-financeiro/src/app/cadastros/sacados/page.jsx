...
import FiltroLateralSacados from '@/app/components/FiltroLateralSacados'; // Importa o novo filtro
...
export default function SacadosPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [sacadosRes, tiposRes] = await Promise.all([
                ...
                fetch(`${API_URL}/tipos-operacao`) // Busca os tipos de operação
            ]);
    ...
    const handleSaveSacado = async (id, data) => { /* ... (sua lógica de salvar) ... */ };
    const handleDeleteRequest = (id) => { const sacado = sacados.find(s => s.id === id); setSacadoParaExcluir(sacado); };
    const handleConfirmarExclusao = async () => { /* ... (sua lógica de excluir) ... */ };
    ...
    return (
        <main className="p-4 sm:p-6 flex flex-col h-full">
            ...
            <EditSacadoModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} sacado={editingSacado} onSave={handleSaveSacado} onDelete={handleDeleteRequest} showNotification={showNotification} tiposOperacao={tiposOperacao} />
            ...
            <div className="flex-grow flex flex-col lg:flex-row gap-6">
                <FiltroLateralSacados
                    filters={filters}
                    onFilterChange={handleFilterChange}
                    onClear={clearFilters}
                    onApply={() => {}} // A filtragem é reativa
                />
